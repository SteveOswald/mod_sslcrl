...
//static void sendto_receivefrom(void *data)
//{
//	int failed;
//	sendto_receivefrom_helper( "127.0.0.1", APR_INET);
//}
//
//static void socket_userdata(void *data)
//{
...
/**
 * Disables SSL for the connection used to download the CRL
 *
 * @param c
 * @return
 */
static int sslcrl_ssl_disable(conn_rec *c) {
  if (sslcrl_m_ssl_disable) {
    return sslcrl_m_ssl_disable(c);
  }
  return 0;
}

/**
 * Enables SSL for the connection used to download the CRL
 *
 * @param c
 * @return
 */
static int sslcrl_ssl_enable(conn_rec *c) {
  if(sslcrl_m_ssl_enable) {
    return c ? sslcrl_m_ssl_enable(c) : 1;
  }
  return 0;
}

/**
* Opens a connection to download the CRL
*
* @param pool
* @param s
* @param host
...
* @param msg Error message if connection can't be opened
* @return
*/
static apr_status_t *sslcrl_connect(apr_pool_t *pool, server_rec *s, const char *host,
	int port, int is_ssl, char **msg, apr_socket_t **socket) {
	apr_sockaddr_t *sa;
	apr_socket_t *newsock;
	apr_status_t rc;

	rc = apr_sockaddr_info_get(&sa, "127.0.0.1", APR_INET, 11000, 0, pool);
	if (rc != APR_SUCCESS) {
		*msg = apr_psprintf(pool, "Failed to resolve host '%s'.", host);
		return rc;
	}

	rc = apr_socket_create(&newsock, sa->family, SOCK_STREAM, APR_PROTO_TCP, pool);
	if (rc != APR_SUCCESS) {
		*msg = apr_pstrdup(pool, "Failed to create new socket.");
		return rc;
	}

	rc = apr_socket_opt_set(newsock, APR_SO_NONBLOCK, 1);
	if (rc != APR_SUCCESS) {
		*msg = apr_pstrdup(pool, "Failed to set APR_SO_NONBLOCK option for socket.");
		return rc;
	}

	rc = apr_socket_timeout_set(newsock, (APR_USEC_PER_SEC * 10));
	if (rc != APR_SUCCESS) {
		*msg = apr_pstrdup(pool, "Failed to set timeout for socket.");
		return rc;
	}

	rc = apr_socket_connect(newsock, sa);
	if (rc != APR_SUCCESS) {
		apr_socket_close(s);
		*msg = apr_psprintf(pool, "Failed to connect to remote server '%s:%d'.", host, port);
		return rc;
	}
...
//static conn_rec *sslcrl_connect(apr_pool_t *pool, server_rec *s, const char *host,
//	int port, int is_ssl, char **msg) {
//	int ssl_status = -1;
//	char *buf = NULL;
//	apr_status_t rc;
//	conn_rec* c;
...
//	sslcrl_host_t *h = sslcrl_resolve(pool, host, port, msg);
//
//	//test();
//
//
...
//		return NULL;
//	}
//	if ((rc = apr_socket_create(&newsock, h->addr->family,
//		SOCK_STREAM, APR_PROTO_TCP, pool)) != APR_SUCCESS) {
//		*msg = apr_pstrdup(pool, "Failed to create new socket.");
//		return NULL;
//	}
//	if ((rc = apr_socket_opt_set(newsock, APR_TCP_NODELAY, 1)) != APR_SUCCESS) {
//		*msg = apr_pstrdup(pool, "Failed to set socket option.");
//		return NULL;
//	}
//	if ((rc = apr_socket_opt_set(newsock, APR_SO_NONBLOCK, 1)) != APR_SUCCESS) {
//		*msg = apr_pstrdup(pool, "Failed to set socket option.");
//		return NULL;
//	}
//	apr_socket_timeout_set(newsock, s->timeout);
//	rc = apr_socket_connect(newsock, h->addr);
//	if (rc != APR_SUCCESS) {
//		apr_socket_close(newsock);
//		*msg = apr_psprintf(pool, "Failed to connect to remote server '%s:%d'.",
//			h->host, h->port);
//		return NULL;
//	}
//	bucket_alloc = apr_bucket_alloc_create(pool);
//	c = ap_run_create_connection(pool, s, newsock, 0, NULL, bucket_alloc);
//	if (!c || c == NULL) {
//		apr_socket_close(newsock);
//		*msg = apr_psprintf(pool, "Failed to create connection to remote server '%s:%d'.",
//			h->host, h->port);
//		return NULL;
//	}
//	if (is_ssl) {
//		if (!sslcrl_ssl_enable(c)) {
//			apr_socket_close(newsock);
//			*msg = apr_psprintf(pool, "Failed to enable ssl support for"
//				" connection to remote server '%s:%d'"
//				" [Hint: enable SSLProxyEngine].",
//				h->host, h->port);
//			return NULL;
//		}
//	}
//	else {
//		ap_log_error(APLOG_MARK, APLOG_INFO, 0, s, SSLCRL_LOG_PFX(040)"Trying to disable SSLEngine for current connection.");
//		//ssl_status = sslcrl_ssl_disable(c);
//
//		if (sslcrl_m_ssl_disable == NULL) {
//			ap_log_error(APLOG_MARK, APLOG_CRIT, 0, s, SSLCRL_LOG_PFX(040)"Couldn't disable SSLEngine because FN isn't registred!");
//		}
//
//		if (ssl_status < 1) {
//			ap_log_error(APLOG_MARK, APLOG_ERR, 0, s, SSLCRL_LOG_PFX(040)"SSLEngine state is '%d'.", ssl_status);
//		}
//		else {
//			ap_log_error(APLOG_MARK, APLOG_INFO, 0, s, SSLCRL_LOG_PFX(040)"SSLEngine state is '%d'.", ssl_status);
//		}
//	}
//	apr_table_set(c->notes, SSLCRL_CONN, "crl");
//	rc = ap_run_pre_connection(c, newsock);
//	if (rc != OK && rc != DONE) {
//		apr_socket_close(newsock);
//		*msg = apr_psprintf(pool, "Connection setup"
//			" to remote server '%s:%d' has failed.",
//			h->host, h->port);
//		return NULL;
//	}
//	if ((rc = apr_socket_opt_set(newsock, APR_TCP_NODELAY, 1)) != APR_SUCCESS) {
...
/**
 * HTTP request to fetch the CRL file
 *
 * @param pool
 * @param c Connection which has been established using sslcrl_connect()
 * @param sconf
 * @param host
 * @param port
 * @param is_ssl true for HTTPS, false for HTTP
 * @param url Path to the CRL
 * @return
 */ 
static int sslcrl_request(apr_pool_t *pool, apr_socket_t *s, sslcrl_config_t *sconf,
                          const char *host, int port,
                          int is_ssl, const char *url) {
	char *buf;
	apr_status_t rc;

	buf = apr_pstrcat(pool, "GET ", url, " HTTP/1.1", CRLF, NULL);
	//buf = apr_pstrcat(pool, buf, "Host: ", host, CRLF, NULL);
	//buf = apr_pstrcat(pool, buf, "Host: ", host, CRLF, NULL);

	//if (sconf->headername) {
	//	buf = apr_pstrcat(pool, buf, sconf->headername, ": ", sconf->headervalue, CRLF, NULL);
	//}

	//buf = apr_pstrcat(pool, buf, "Connection: close" CRLF, NULL);
	//buf = apr_pstrcat(pool, buf, "User-Agent: ", ap_get_server_banner(), CRLF, NULL);
	//buf = apr_pstrcat(pool, buf, CRLF, NULL);

	rc = apr_socket_send(s, buf, strlen(buf));
	if (rc != APR_SUCCESS) {
		return rc;
	}
...
//
//static int sslcrl_request(apr_pool_t *pool, conn_rec *c, sslcrl_config_t *sconf,
//	const char *host, int port,
//	int is_ssl, const char *url) {
...
//	e = apr_bucket_pool_create(buf, strlen(buf), pool, c->bucket_alloc);
//	APR_BRIGADE_INSERT_TAIL(bb, e);
//	buf = apr_pstrcat(pool, "Host: ", host, CRLF, NULL);
//	if (sconf->headername) {
//		buf = apr_pstrcat(pool, buf, sconf->headername, ": ", sconf->headervalue, CRLF, NULL);
//	}
//	buf = apr_pstrcat(pool, buf, "Connection: close" CRLF, NULL);
...
//	e = apr_bucket_pool_create(buf, strlen(buf), pool, c->bucket_alloc);
//	APR_BRIGADE_INSERT_TAIL(bb, e);
//	buf = apr_pstrcat(pool, CRLF, NULL);
//	e = apr_bucket_pool_create(buf, strlen(buf), pool, c->bucket_alloc);
//	APR_BRIGADE_INSERT_TAIL(bb, e);
//	e = apr_bucket_flush_create(c->bucket_alloc);
//	APR_BRIGADE_INSERT_TAIL(bb, e);
//	rc = ap_pass_brigade(c->output_filters, bb);
//	if (rc != APR_SUCCESS) {
//		if (c->aborted) {
//			return APR_STATUS_IS_TIMEUP(rc) ? HTTP_GATEWAY_TIME_OUT : HTTP_BAD_GATEWAY;
//		}
//		else {
 ...
 * Receives the CRL
 */
static apr_status_t sslcrl_response(apr_pool_t *pool, conn_rec *c, 
                                    const char *host, char **msg,
                                    char **status_line,
  ...
  char *buf; 
  apr_size_t len;
  apr_bucket_brigade *bb = apr_brigade_create(pool, c->bucket_alloc);
  apr_bucket_brigade *body_bb = apr_brigade_create(pool, c->bucket_alloc);
  int eos = 0;
  request_rec *rp = sslcrl_make_fake_req(c, pool);
  rp->proxyreq = PROXYREQ_RESPONSE;
  *headers_in = rp->headers_in;
  *status_line = NULL;
  *body = NULL;
  *body_len = 0;
  *msg = NULL;
  /* response status */
  rc = ap_get_brigade(c->input_filters, bb, AP_MODE_GETLINE, APR_BLOCK_READ, HUGE_STRING_LEN);
  if(rc != APR_SUCCESS) {
    *msg = apr_psprintf(pool, "Failed to read status line from '%s'"
                        " [Hint: check SSL settings].", host);
    return rc;
  }
  apr_brigade_pflatten(bb, &buf, &len, pool); 
  total = total + len;
  apr_brigade_cleanup(bb);
  if(len == 0 || !apr_date_checkmask(buf, "HTTP/#.# ###*")) {
    *msg = apr_psprintf(pool, "Invalid status line from '%s'.", host);
    return rc;
  } else {
    char *e;
    rp->status_line = apr_pstrdup(pool, &buf[9]);
    e = (char *)rp->status_line;
    while(e && e[0]) {
      if(e[0] < 32) {
        e[0] = '\0';
      } else {
    ...
    if(strncmp(rp->status_line, "200", 3) != 0) {
      *msg = apr_psprintf(pool, "Request to '%s' failed: '%s'.", host, rp->status_line);
      status = HTTP_BAD_REQUEST;       
    }
    buf[12] = '\0';
    rp->status = atoi(&buf[9]);
  }
  /* headers */
  do {
    rc = ap_get_brigade(c->input_filters, bb, AP_MODE_GETLINE, APR_BLOCK_READ, HUGE_STRING_LEN);
    if(rc == APR_SUCCESS) {
      ...
      apr_brigade_cleanup(bb);
      if(len > 0) {
        char *raw = apr_pstrndup(pool, buf, len);
        if(raw[0] == '\r' && raw[1] == '\n') {
          break;
        } else {
          char *next;
          char *name = apr_strtok(raw, ":", &next);
          char *value = apr_strtok(NULL, ": ", &next);
          if(name && value) {
            char *c = value;
            while(c[0]) {
              if(c[0] == '\r' || c[0] == '\n') {
            ...
            apr_table_set(rp->headers_in, name, value);
          } else {
            break;
          }
        }
    ...
    if(apr_table_elts(rp->headers_in)->nelts > 30) {
      *msg = apr_psprintf(pool, "%sReceived more than 30 response headers from '%s'.",
                          *msg ? apr_pstrcat(pool, *msg, " ", NULL) : "", host);
      return HTTP_BAD_REQUEST;       
    }
  } while(rc == APR_SUCCESS);
  if(rc != APR_SUCCESS) {
    *msg = apr_psprintf(pool, "%sFailed to read response headers from '%s'.",
                        *msg ? apr_pstrcat(pool, *msg, " ", NULL) : "", host);
    return HTTP_BAD_REQUEST;
  }
  /* body */
  ap_add_input_filter("HTTP_IN", NULL, rp, c);
  do {
    if((rc = ap_get_brigade(rp->input_filters, bb, AP_MODE_READBYTES,
      ...
      for(b = APR_BRIGADE_FIRST(bb); b != APR_BRIGADE_SENTINEL(bb); b = APR_BUCKET_NEXT(b)) {
        ...
        if(!(APR_BUCKET_IS_METADATA(b))) {
          const char *buf;
          apr_size_t nbytes;
          if((rc = apr_bucket_read(b, &buf, &nbytes, APR_BLOCK_READ)) == APR_SUCCESS) {
            apr_brigade_write(body_bb, NULL, rp, buf, nbytes);
            total = total + nbytes;
            if(total > SSLCRL_MAX_RES_LEN) {
              *msg = apr_psprintf(pool, "%sReceived more than %d bytes from '%s'.",
                                  *msg ? apr_pstrcat(pool, *msg, " ", NULL) : "",
                                  SSLCRL_MAX_RES_LEN, host);
              return HTTP_BAD_GATEWAY;
            }
          } else {
            *msg = apr_psprintf(pool, "%sFailed to read data from '%s'.",
                                *msg ? apr_pstrcat(pool, *msg, " ", NULL) : "",
                                host);
          }
        }
      ...
      *msg = apr_psprintf(pool, "%sFailed to get data from '%s' (%d).",
                          *msg ? apr_pstrcat(pool, *msg, " ", NULL) : "",
                          host, rc);
    }
  } while((rc == APR_SUCCESS) && !eos);
  apr_brigade_pflatten(body_bb, &buf, &len, pool);
  *body = buf;
  *body_len = len;
  if(eos) {
    return status;
